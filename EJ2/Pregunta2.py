#El problema se basa en implementar una clase RandomizedSet la cual debe inicializar un objeto, insertar un valor si no existe
#y devolver un boolean representando True si se agrego y False si no se agrego, asi mismo se debe eliminar valores y devolver 
#boolean para confirmar, finalmente devolver un valor aleatorio de los que se encuentren en el set. 
#Para esto crearemos una clase con las funciones de insertar, eliminar y obtener un numero aletorio.
#Para la funcion de insertar revisaremos si no existe el numero con un condicionador y proceder a agregarlo en caso de que no exista
#con sus respectivos returns de booleans confirmando las acciones
#Exactamente lo mismo con la funcion de eliminar el valor y para la funcion de obtener un numero aleatorio dentro del set
#Se usa la biblioteca random y se busca dentro del set.
#Para probar que el ejercicio funciona aplicamos una funcion unittest para confirmar que las respuestas sean correctas e
#hicimos 5 sets, agregando eliminando y buscando de diferentes formas
